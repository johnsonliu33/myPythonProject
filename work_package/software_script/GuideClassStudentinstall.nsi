...
; 安装程序初始定义常量
!define PRODUCT_NAME "双师精品课系统 - 学生端"
!define PRODUCT_VERSION "1.0.2.1906"
!define PRODUCT_PUBLISHER "简单科技"
!define PRODUCT_WEB_SITE "http://www.jd100.com/"
!define PRODUCT_DIR_REGKEY "Software\Microsoft\Windows\CurrentVersion\App Paths\GuideClassStudentInstaller.exe"
...
; 安装完成页面
!define MUI_FINISHPAGE_RUN "$LOCALAPPDATA\Easytech\GuideClassStudent\GuideClassStudent.exe"
...
;产品名称
VIAddVersionKey /LANG=${LANG_SimpChinese} "ProductName" "双师精品课系统"
;公司
VIAddVersionKey /LANG=${LANG_SimpChinese} "CompanyName" "北京简单科技有限公司"
;版权
VIAddVersionKey /LANG=${LANG_SimpChinese} "LegalCopyright" ""
;描述
VIAddVersionKey /LANG=${LANG_SimpChinese} "FileDescription" "双师精品课系统 - 学生端"
;文件版本号
VIAddVersionKey /LANG=${LANG_SimpChinese} "FileVersion" "1.0.2.1906"
; 安装预释放文件
...
;输出文件名
OutFile "GuideClassStudentSetup1.0.2.1906.exe"
;安装目录
InstallDir "$LOCALAPPDATA\Easytech\GuideClassStudent"
;安装目录写入注册表
;InstallDirRegKey HKLM "${PRODUCT_UNINST_KEY}" "UninstallString"
;显示安装详细进度
ShowInstDetails show
;左下角标识
BrandingText "双师精品课安装程序"

RequestExecutionLevel user
Section "双师精品课" SEC01
;
  SetOutPath "$LOCALAPPDATA\Easytech\GuideClassStudent"
  SetOverwrite on
  ;卸载之前版本部分
 ...
 ;结束进程
  KillProcDLL::KillProc "GuideClassStudent.exe"
  ...
  WriteRegStr HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\GuideClassStudent" "DisplayIcon" "$INSTDIR\GuideClassStudent.exe"
  WriteRegStr HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\GuideClassStudent" "DisplayName" "${PRODUCT_NAME}"
  WriteRegStr HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\GuideClassStudent" "DisplayVersion" "${PRODUCT_VERSION}"
  WriteRegStr HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\GuideClassStudent" "Publisher" "${PRODUCT_PUBLISHER}"
  WriteRegStr HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\GuideClassStudent" "UninstallString" "$INSTDIR\GuideClassStudentInstaller.exe uninstall"
  WriteRegDWORD HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\GuideClassStudent" "EstimatedSize" ${INSTALLSIZE}
  ...
  ;创建开始菜单快捷方式
  CreateDirectory "$SMPROGRAMS\简单学习网\双师精品课(学生端)"
  CreateShortCut "$SMPROGRAMS\简单学习网\双师精品课(学生端)\双师精品课(学生端).lnk" "$INSTDIR\GuideClassStudent.exe"
  CreateShortCut "$SMPROGRAMS\简单学习网\双师精品课(学生端)\卸载双师精品课(学生端).lnk" "$INSTDIR\GuideClassStudentInstaller.exe" uninstall
  
  ;创建桌面快捷方式
  CreateShortCut "$DESKTOP\双师精品课(学生端).lnk" "$INSTDIR\GuideClassStudent.exe"
